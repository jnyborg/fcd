...
class CAMSegmentationModel(torch.nn.Module):
    ...
    def cam(self, x, class_idx=None):
        with torch.no_grad():
            ...
            # Compute CAM
            cam = F.conv2d(features, self.classification_head[-1].weight)
            cam = F.relu(cam)
...
class UCAM(CAMSegmentationModel):
    """Modified UNet with CAM output and GAP classifier attached to decoder
    ...
    def __init__(
        ...
        self.classification_head = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(decoder_channels[-1], classes, 1, bias=False)
        )
